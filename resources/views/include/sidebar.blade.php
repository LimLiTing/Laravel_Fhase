    ...
    <div class="sidebar-content">
        <div class="nav-container">
            <nav id="main-menu-navigation" class="navigation-main">
                ...
                <div class="nav-item {{ ($segment1 == 'upload-file' || $segment1 == 'fileList') ? 'active open' : '' }} has-sub">
                    <a href="#"><i class="ik ik-alert-circle"></i><span>{{ __('Blank')}}</span></a>
                    <div class="submenu-content">
                        <!-- upload file -->
                        @can('manage_upload')
                        <a href="{{url('upload-file')}}" class="menu-item {{ ($segment1 == 'upload-file') ? 'active' : '' }}">{{ __('Upload File')}}</a>
                         @endcan
                         <!-- view upload file list -->
                        @can('manage_file')
                        <a href="{{url('fileList')}}" class="menu-item {{ ($segment1 == 'fileList') ? 'active' : '' }}">{{ __('File List')}}</a>
                        @endcan
                    </div>
                </div>
                <div class="nav-item {{ ($segment1 == 'userProfile' || $segment1 == 'editProfile') ? 'active open' : '' }} has-sub">
                    <a href="#"><i class="ik ik-user-check"></i><span>{{ __('User')}}</span></a>
                    <div class="submenu-content">
                        <!-- Display Profile -->
                        @can('manage_profile')
                        <a href="{{url('userProfile')}}" class="menu-item {{ ($segment1 == 'userProfile') ? 'active' : '' }}">{{ __('User Profile')}}</a>
                         @endcan
                         <!-- Edit Profile -->
                        @can('manage_edit')
                        <a href="{{url('editProfile')}}" class="menu-item {{ ($segment1 == 'editProfile') ? 'active' : '' }}">{{ __('Edit Profile')}}</a>
                        @endcan
                    </div>
                </div>
                <div class="nav-item {{ ($segment1 == 'pos') ? 'active' : '' }}">
